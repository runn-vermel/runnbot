...
`;
  ...
  var writeToFile = function() {
    var currentDate = new Date(),
        ...
        ReportName = currentDate.getFullYear() + "-" + currentDate.getMonth() + "-" + currentDate.getDate() + "-" + currentDate.getHours() + currentDate.getMinutes() + "-" + dryOrLive + '.txt',
        lastSlash = __dirname.lastIndexOf("/"),
        reportPath = __dirname.substr(0, lastSlash) + "/reports/" + ReportName;
    return fs.open(reportPath, "wx", (err, fd) => fs.writeFile(reportPath, line, (err, res) => Promise.resolve(reportPath)));
  };
