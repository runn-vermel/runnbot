...
/**
 * removes the seed by default.
 * @param  {[Array]} dirList [a list of dirs]
 * @return {[Promise]}         [a list of dirs without the seed]
 */
  var removeSeed = function(dirList) {
    return Promise.filter(dirList, (dir) => {
      var lastSlash = dir.lastIndexOf("/"),
          dirName = dir.substr(lastSlash+1);
      return dirName !== "predix-seed";
    });
  };
  /**
  ...
  */
  ...
  var main = function() {
    if (shared.requestedRepos.length) {
      ...
      .then((dirList) => (shared.includeSeed) ? Promise.resolve(dirList) : removeSeed(dirList))
      .then((dirList) => Promise.resolve(dirList));
    }
  };
